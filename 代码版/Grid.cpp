...
bool Grid::s_textSettingsInitialized = false;

// 加载格子图片资源
bool Grid::LoadImages(int cellSize) {
    if (s_imagesLoaded) {
        // 已加载过，但仍需重置文本设置标志
        s_textSettingsInitialized = false;
        return true; 
    }
    ...
    try {
        ...
        s_imagesLoaded = true;
        s_textSettingsInitialized = false; // 确保文本设置被重置
        return true;
    } catch (...) {
        // 如果加载失败，显示错误消息
...
void Grid::draw(int x, int y, bool isGameOver, bool isWin, bool showMine) const {
    ...
    if (isOpened()) {
        ...
        // 调试模式下，显示地雷位置
        if (showMine && isMine()) {
            // 设置文本属性（这些设置一般比绘制更耗时，尽量减少不必要的设置）
            if (!s_textSettingsInitialized) {
                setbkmode(TRANSPARENT);
                settextcolor(RGB(255, 0, 0));  // 红色
                settextstyle(20, 0, _T("宋体"), 0, 0, FW_NORMAL, false, false, false);
                s_textSettingsInitialized = true;
            } else {
                // 每次显示雷字时都强制设置为红色文字，确保颜色正确
                settextcolor(RGB(255, 0, 0));  // 红色
            }
